...
def bbox_iou(
    ...
    if CIoU or DIoU or GIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            # if CIoU:  # CIoU https://github.com/Zzh-tju/DIoU-SSD-pytorch
            #     v = (4 / math.pi ** 2) * torch.pow(torch.atan(w2 / h2) - torch.atan(w1 / h1), 2)
            #     with torch.no_grad():
            #         alpha = v / (v - iou + (1 + eps))
            #     ciou = iou - (rho2 / c2 + v * alpha)  # 计算出原版的 CIoU
            #     # ========================================================
            #     # 🚀 论文创新点：加入 NWD (Normalized Wasserstein Distance)
            #     # ========================================================
            #     # rho2 已经是两个框中心点距离的平方，w1/h1 是预测框宽高，w2/h2 是真实框宽高
            #     # 计算 Wasserstein 距离的平方 (中心点距离 + 宽高协方差距离)
            #     w2_dist = rho2 + ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4
            #     # 计算 NWD。常数 C 取 12.8 是学术界在 VisDrone 等无人机数据集上的最佳实践经验值
            #     nwd = torch.exp(-w2_dist / 12.8)
            #     # 联合损失输出：一半依赖 CIoU 保证大目标稳定，一半依赖 NWD 狂拉小目标
            #     return 0.5 * ciou + 0.5 * nwd
