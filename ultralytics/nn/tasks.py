...
from ultralytics.nn.modules import (
    ...
    YOLOEDetect,
    YOLOESegment,
    v10Detect, PConv, EMA, UAV_LiteBlock, C3_UAV,
)
from ultralytics.nn.modules.block import SPDConv, C3_SGRF, CGFA, GhostC3k2,BiFPN_Fusion
from ultralytics.nn.modules.head import SharedDetect
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # >>>>> 创新点插入：BiFPN_Fusion 解析逻辑 >>>>>
        elif m is BiFPN_Fusion:
            c2 = ch[f[0]]  # 输出通道数自动对齐到第一个输入分支的通道数
            args = [[ch[x] for x in f]]  # 把所有输入分支的通道数打包传给 __init__
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        elif m is CGFA:
            c2 = sum(ch[x] for x in f)  # 输出通道数依然是拼接后的总和
            args = [[ch[x] for x in f]]  # 将两个输入特征的通道数打包成列表传给 CGFA 的 __init__
        elif m in frozenset(
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, SharedDetect}
        ):
