...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SwiGLUFFN, EMA
...
# ==========================================
# 本文创新点：跨尺度权重共享解耦检测头 (SharedDetect)
# 作用：强制 P3、P4、P5 三个预测分支共享卷积核参数，断崖式降低模型体积
# ==========================================
...
class SharedDetect(Detect):
    def __init__(self, nc=80, ch=()):
        ...
        assert len(set(ch)) == 1, "SharedDetect 要求所有输入特征图的通道数必须严格相同"

        # 1. 内存地址复用 (权重共享)
        for i in range(1, self.nl):
            self.cv2[i] = self.cv2[0]
            self.cv3[i] = self.cv3[0]

            # 2. 自适应尺度缩放因子 (刚性保护)
        self.scale_factors = nn.Parameter(torch.ones(self.nl, 1, 1, 1))

        # 3. 专属小目标的 EMA 特征提纯 (柔性保护)
        # 仅对第一个输入分支（P3 浅层）实例化一个 EMA 模块
        self.ema_p3 = EMA(ch[0])

    def forward(self, x):
        shape = x[0].shape

        # 🚨 核心创新：在进入共享卷积前，对 P3 浅层特征进行 EMA 注意力提纯
        # 这样就能完美抵消轻量化带来的小目标精度流失！
        x[0] = self.ema_p3(x[0])

        for i in range(self.nl):
            box = self.cv2[i](x[i]) * self.scale_factors[i]
            cls = self.cv3[i](x[i])
        ...
        # 💡 官方标准接口：在验证/推理模式下，必须返回 (预测结果, 原始特征图) 的元组！
        return y if self.export else (y, x)
